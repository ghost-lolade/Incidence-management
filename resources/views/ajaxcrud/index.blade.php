          ...
          <table class="table table-bordered" id="laravel_crud" width="80%">
           ...
           <tbody id="posts-crud">
              ...
              <tr id="post_id_{{ $post->id }}">
                 ...
                 <td>{{ $post->atm_name }}</td>
				 <td>{{ $post->subject }}, {{ $post->error_code }}</td>
                 <td>{{ $post->vendor_name }}</td>
				 <td>{{ $post->request_status }}</td>
				 <td>{{ $post->closed_at }}</td>
				 <td>{{ $post->part_replaced }}</td>
										
                 <td><a href="javascript:void(0)" id="edit-post" data-id="{{ $post->id }}" class="btn btn-info">Confirm</a></td>
                 <td>
