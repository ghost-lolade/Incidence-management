    ...
    <section class="content">
        <div class="box">
            <div class="box-header">
                <div class="row">
                    <div class="col-sm-8">
                        <h3 class="box-title">Confirm Call Closure  :: {{$date = date('Y-m-d H:i:s')}}</h3>
                    </div>
                    <div class="col-sm-4">
                    ...
                    <tr class="item{{$post->id}}">
                        ...
                        <td>
                            <button class="show-modal btn btn-success" data-id="{{$post->id}}" data-title="{{$post->terminal_id}}" data-content="{{$post->address}} -- {{$post->custodian_phone}}-- {{$post->decline_reason}}"
                                    data-assign="{{$post->ce_name}}" data-status="{{$post->ce_status}}" data-timer="{{$post->ce_arrival_time}}" data-suspend="{{$post->suspend_comment}}">
                                <span class="glyphicon glyphicon-eye-open"></span> </button>
                            <button class="edit-assignmodal btn btn-info" data-id="{{$post->id}}" data-title="{{$post->terminal_id}}">
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <form class="form-horizontal" role="form">
                                    ...
                                    <div class="form-group">
                                        <label class="control-label col-sm-3" for="title">Error Discription:</label>
                                        <div class="col-sm-9">

                                            <input class="form-control" id="errorcode_edit" placeholder="Error 2:1" required>
                                            </input>
...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if ((data.errors)) {
                    ...
                    $('.item' + data.id).replaceWith("<tr class='item" + data.id + "'><td class='col1'>" + data.id + "</td><td>" + data.terminal_id + "</td><td>" + data.atm_name + "</td><td>" + data.error_code + "</td><td>" + data.request_status + "</td><td>" + data.ce_name + "</td><td>" + data.log_day + "</td><td><button class='show-modal btn btn-success' data-id='" + data.id + "' data-title='" + data.title + "' data-content='" + data.content + "'><span class='glyphicon glyphicon-eye-open'></span> Show</button> <button class='edit-assignmodal btn btn-info' data-id='\" + data.id + \"' data-title='\" + data.title + \"' data-content='\" + data.content + \"'><span class='glyphicon glyphicon-plus-sign'></span> Update Error</button> <button class='edit-suspendcemodel btn btn-warning' data-id='" + data.id + "' data-title='" + data.title + "' data-content='" + data.content + "'><span class='glyphicon glyphicon-tint'></span> Suspend Call</button> <button class='delete-modal btn btn-primary' data-id='" + data.id + "' data-title='" + data.title + "' data-content='" + data.content + "'><span class='glyphicon glyphicon-ok-circle'></span> Close Call</button></td></tr>");
    ...
    $(document).on('click', '.edit-modal', function() {
        $('.modal-title').text('Suspend/False Call');
        $('#id_edit').val($(this).data('id'));
        $('#title_edit').val($(this).data('title'));
        $('#content_edit').val($(this).data('suspend_comment'));
        $('#status_call').val($(this).data('r_status'));
        id = $('#id_edit').val();
