...
//        mail($this->from_email, $this->subject, $newmsg, $headers);
   ...
   Public function sendRequesterMail() {
	...
	foreach($select AS $row) {
	$sn= $row['sn'];
	$ce_name= $row['request_assign_to'];
	//$ce_email = $row['tech_email'];
	$ce_phone =$row['tech_phone'];
	...
	// Add leading Zeros to the S/N
	 $lastsn = sprintf( '%08d', $lastsn);

	 // Generate the Table with Data to Customer 
	// Build table rows...
        $msg.= "<tr><td><b>$ticket_no</b></td><td>$atm_name</td>x<td>$ce_name</td><td>$ce_phone </td><td>$terminal_id </td><td>$created_date</td><td>$created_time</td></tr>";
 	}
   // Close the table
$msg.= "</table>";


        $select = $this->pdo->query("SELECT requester_email FROM tbl_requester_dump");
        $select = $select->fetchAll();
        if(count($select) > 0) {
		...
		$newmsg .= "<br>";
           ...
           "Content-type: text/html; charset=UTF-8" . "\r\n";
                ...
                mail($recipient["requester_email"], $this->subject, $newmsg, $headers);
            }
        } else {
