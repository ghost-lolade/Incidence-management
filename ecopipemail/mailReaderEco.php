   ...
   var $tech_name;
    ...
    private function saveToDb() {
        ...
        if (!$insert->execute(Array(
                    ...
                    mb_convert_encoding($this->bodyhtml, 'UTF-8', 'UTF-8')
                ))) {
            if ($this->debug) {
	...
	//UNIVERSAL HORIZON LTD ATM Fault Log Closure 10 Jul 2018

	
preg_match('/Closure/', $this->subject, $match);
if ($match[0] == 'Closure') {
    echo " Discard Call is a closed call";
	 die();
} else
	{
	$matches = array();
		// Stop the mail after seeing "From" Dont read mail tray
		$result = explode("From2:",$this->bodyhtml);
		$remove_from= $result[0];
		$pieces = explode(PHP_EOL, $remove_from);
		
		//$pieces = explode(PHP_EOL, $this->body);
		
		$matches = preg_grep("/1050/", $pieces);
	...
	*/
	...
	$this->sla_level =$row['sla_level'];
	$this->sla_hour =$row['hour'];
	$this->region =$row['region'];
	$this->brand =$row['brand'];
	 	}
	 		 	
	 	
	 $select = $this->pdo->query("SELECT * FROM call_logs1  WHERE terminal_id = $term AND request_status ='Open' ");
     $select = $select->fetchAll();
 	 if(count($select) > 0) {
	...
	$select_tech = $this->pdo->query("SELECT * FROM technicians WHERE terminal_id = $term");
	...
	if ($this->day== 'Monday' ||$this->day== 'Tuesday' ||$this->day== 'Wednesday' ||$this->day == 'Thursday' ||$this->day== 'Friday' ){
        ...
        if ($this->timers ==1 && $this->times >= 0000 && $this->times <= 758): 
             $this->due_date= date("Y-m-d 14:00:00",strtotime('today'));
       
        elseif ($this->timers ==1 && $this->times>= 759 && $this->times<= 1100):    	 
        	 $this->due_date= date('Y-m-d H:i:s', strtotime('+7 hours', $this->today));
        	 
        //To be closed by  the following day
        elseif ( $this->timers ==1 && $this->times >= 1101 && $this->times <= 1700):           
          $this->due_date= date('Y-m-d H:i:s', strtotime('+21 hours', $this->today));
       
        elseif ($this->timers ==1 && $this->times >= 1701 && $this->times <= 2359): //All call above 5pm should be postpone to second day
             $this->due_date= date("Y-m-d 14:00:00",strtotime('tomorrow'));
             
       // For 10 Hours SLA
       elseif ($this->timers ==2 && $this->times>= 0000 && $this->times<= 759 ):    	 
        	 $this->due_date= date('Y-m-d H:i:s', strtotime('+10 hours', $today));
        	 
        //To be closed by the following day
        elseif ($this->timers ==2 && $this->times >= 800 && $this->times <= 1659):           
          $this->due_date= date('Y-m-d H:i:s', strtotime('+25 hours', $this->today));
       
        elseif ($this->timers ==2 && $this->times >= 1701 && $this->times <= 2359): //All call above 5pm should be postpone to second day
  ...
  /*  $select_tech = $this->pdo->query("Select * from tbl_tech_email where terminal_id = $mat ");
    ...
    */    
    	 ...
    	 $insertRequest = $this->pdo->prepare("INSERT INTO call_logs1(fromaddress,subject,body,terminal_id,atm_state,ce_name,ce_phone,address,ticket_no,mail_at,log_day,log_time,atm_name,request_status,vendor_request_status,vendor_id,vendor_name,sla_level,region,brand) VALUES 
		 (:fromaddress,:subject,:body,:terminal_id,:atm_state,:ce_name,:ce_phone,:address,:ticket_no,:mail_at,:log_day,:log_time,:atm_name,:request_status,:vendor_request_status,:vendor_id,:vendor_name,:sla_level,:region,:brand)");
	...
	//    $insertRequest->bindParam(':due_at', $this->due_date);
	    ...
	    $insertRequest->bindParam(':log_time', $this->mailtime);
	     $insertRequest->bindParam(':atm_name',  $this->atm_name);
		  ...
		  $insertRequest->bindParam(':vendor_request_status', $this->request_status);
	      
		  $insertRequest->bindParam(':vendor_id', $this->vendor_id);
		  $insertRequest->bindParam(':vendor_name', $this->vendor_name);
		  $insertRequest->bindParam(':sla_level', $this->sla_level);
		//  $insertRequest->bindParam(':hour', $this->sla_hour);
		  $insertRequest->bindParam(':region', $this->region);
		  $insertRequest->bindParam(':brand', $this->brand);
	   ...
	   //         $this->lastsn='10';
            ...
            */
            ...
            echo "No Terminal ID to Need to Save to call_logs1 ";
            die();
        }
...
//Display the message sent
 $headers = 'From: support@eco.uhlng.com' . "\r\n" .
    		'Reply-To: support@eco.uhlng.com' . "\r\n" .
...
//        mail($this->from_email, $this->subject, $newmsg, $headers);
   ...
   Public function sendRequesterMail() {
	...
	$matches = array();
		...
		$this->today = time();
		$this->maildate= date("Y-m-d G.i:s", strtotime('-3 minutes', $this->today)); //Convert to Nigeria time
    
 	$select = $this->pdo->query("Select * from call_logs1 WHERE mail_at >=  '$this->maildate' ");
        $select = $select->fetchAll();
                if(count($select) > 0) {
        
//CE TABLE 
    $msgce = "The Customer Engineer Detail: \n\n";
	$msgce.= "<table  width=100% border=1>";
	// Table header...
	$msgce.= "<thead><tr><th>Ticket ID </th><th>ATM Name</th><th>CE Name </th><th>Terminal ID</th><th>Log Day</th><th>Log Time</th><th>SLA TYPE</th><th>Given Hour</th></tr></thead>\n";
	// Open the table body
	$msgce.= "<tbody>\n";
	...
	// Build table rows...
        $msgce.= "<tr><td><b>$ticket_no</b></td><td>$atm_name</td><td>$ce_name</td><td>$terminal_id </td><td>$created_date </td><td>$created_time</td><td>$sla_level</td><td>$hour</td></tr>";
...
$msgce.= "</table>";
    ...
    // Mail to CE       
        $select = $this->pdo->query("SELECT email FROM uhl_report_email");
	    // $select = $this->pdo->query("SELECT email FROM requesters");
        $select = $select->fetchAll();
        if(count($select) > 0) {
            foreach($select AS $recipients) {
           	$newmsg = "Dear CE, \n\n ";
	...
	$newmsg .= "<p>";
           ...
           "Content-type: text/html; charset=UTF-8" . "\r\n";
                ...
                //mail($recipients["oladejisteven@gmail.com"], $this->subject, $newmsg, $headers);
				 mail($recipients["email"], $this->subject, $newmsg, $headers);
            }
        } else {
            echo "No user to send email";
            die();
