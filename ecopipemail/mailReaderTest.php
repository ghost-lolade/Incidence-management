...
class mailReaderTest {
    ...
    var $send_ce_email = FALSE; // Send Mail to SoftWork Assigned?
...
if ($match[0] == 'Closure') {
  ...
  /*  $select_tech = $this->pdo->query("Select * from tbl_tech_email where terminal_id = $mat ");
    ...
    */    
    	 ...
    	 $insertRequest = $this->pdo->prepare("INSERT INTO call_logs(fromaddress,subject,body,terminal_id,atm_state,ce_name,ce_id,ce_phone,address,ticket_no,due_at,mail_at,log_day,log_time,atm_name,request_status,vendor_id,vendor_name,sla_level,hour,region,brand) VALUES 
		 (:fromaddress,:subject,:body,:terminal_id,:atm_state,:ce_name,:ce_id,:ce_phone,:address,:ticket_no,:due_at,:mail_at,:log_day,:log_time,:atm_name,:request_status,:vendor_id,:vendor_name,:sla_level,:hour,:region,:brand)");
...
//Display the message sent
 $headers = 'From: helpdesk@erp.uhlng.com' . "\r\n" .
    		'Reply-To: helpdesk@erp.uhlng.com' . "\r\n" .
...
$msgce.= "</table>";
        ...
        if(count($select) > 0) {
           ...
           //	$newmsg = "Dear  ". $username . ", <br\> ";
            $newmsg = "TESTING! Your mail has been forwarded to the customer engineer in-charge.. ";
        $newmsg .= "He will be on site soonest ";
		$newmsg .= "<p>";
		$newmsg .= "<br>";
		$newmsg .=$msg . "<br\>";
