...
if ($match[0] == 'Closure') {
	...
	*/
	...
	$this->brand =$row['brand'];
	 ...
	 $select = $this->pdo->query("SELECT * FROM call_logs  WHERE terminal_id = $term AND request_status ='Open' ");
     $select = $select->fetchAll();
 	 if(count($select) > 0) {
  ...
  /*  $select_tech = $this->pdo->query("Select * from tbl_tech_email where terminal_id = $mat ");
    ...
    */    
    	 ...
    	 $insertRequest = $this->pdo->prepare("INSERT INTO call_logs(fromaddress,subject,body,terminal_id,atm_state,ce_name,ce_phone,address,ticket_no,mail_at,log_day,log_time,atm_name,request_status,vendor_request_status,vendor_id,vendor_name,sla_level,region,brand) VALUES 
