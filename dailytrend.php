...
$sql_count = "SELECT Count(*) As counter FROM call_logs WHERE request_status='Open' AND mail_at < '$today_date' ";
...
$sql_count = "SELECT COUNT(*) FROM call_logs WHERE  request_status ='Open'";
...
$text .= "Number of OPEN Calls = " .$open_bf. "\n\n";
$text .= "<p>";
...
//		elseif ($age >96 && $age <=107) 
  ...
  $text .= "<tr><td> $no</td><td>$terminal_id</td><td>$atm_name</td><td>$error_code</td>
		<td> $request_assign_to</td><td>$log_day $log_time</td><td>$daysAge</td><td>$hours</td><td>$brand</td><td>$sla_level</td><td>$atm_state</td>
		</tr>";
